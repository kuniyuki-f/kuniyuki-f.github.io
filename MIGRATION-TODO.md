...
- [ ] 目視比較の最終確認（レスポンシブ含む）— ユーザー自身の目視確認を推奨
  ...
  - 残る人間の確認: デザインの見た目の妥当性（余白・配色・崩れ）はスクショ目視で。既知の未修正: **ホームのServiceグリッドがモバイルで横はみ出し**（`src/pages/index.astro` のインライン `grid-template-columns:12.5rem×3` がレスポンシブ用クラスを打ち消している。E2Eが検出）
